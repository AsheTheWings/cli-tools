...
@click.argument(
    "input_path", type=click.Path(exists=True, path_type=Path), required=False
)
@click.option(
    "-o",
    "--output",
...
def extraxt_handoff_command(
    ...
    if input_path is None:
        ...
        if not resolved_path:
            raise click.ClickException(
                f"No generation input JSON file found in {DEFAULT_TMP_DIR}. Please specify an explicit path."
            )
    ...
    try:
        ...
        raise click.ClickException(
            f"Failed to read/parse input file {input_path}: {exc}"
        )
    ...
    if not raw_text:
        raise click.ClickException(
            "Could not find any handoff or Checkpoint Summary markers within the input file."
        )
